DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `RealizarVentaPorFecha`(
...
BEGIN
    ...
    AND fecha_inventario_producto = p_fecha_venta;
    -- Verificar si la cantidad final es menor o igual a 0 y actualizar el estado del inventario
    IF (SELECT cantidad_final_producto FROM inventario_producto WHERE id_inventario_producto = p_id_detalle_producto AND fecha_inventario_producto = p_fecha_venta) <= 0 THEN
        UPDATE inventario_producto
...
END$$
DELIMITER ;
CALL RealizarVenta('2023-05-29', p_id_cliente, 1, p_cantidad_producto, p_precio_por_unidad, p_descuento_producto);
